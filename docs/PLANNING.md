...
* **Observaciones**:
  ...
  - **Bug reportado por el usuario al probar el login como admin, causa NO relacionada al código**: `TypeError: getColumns is not a function` en `app/admin/page.tsx`. El código en disco estaba correcto (`getColumns` bien exportado, único import, `git log` confirmó el commit). Causa real: el servidor `pnpm dev` del usuario llevaba corriendo desde el 21/7 a las 22:51 — **casi 3 días antes** del commit que introdujo `getColumns` (24/7). El Fast Refresh de Next.js no reconcilió bien el cambio de forma del export (de `const columns` a `function getColumns`). Se reinició el servidor y quedó verificado: login OK, `/admin` y `/admin/doctors` responden `200` sin error. **Lección operativa** (ya la habíamos visto con `middleware.ts` en TASK-005): cualquier servidor de dev que viene corriendo de sesiones anteriores puede quedar desincronizado tras cambios de forma de exports/middleware — ante un error que no tiene explicación en el código, verificar hace cuánto arrancó el proceso (`Get-CimInstance Win32_Process`) contra la fecha del último commit relevante, antes de asumir que es un bug real.
